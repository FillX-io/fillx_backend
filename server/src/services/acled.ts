...
// Returns protest events from the last 30 days
...
export async function fetchAcledProtests(): Promise<AcledResult> {
  const now = Date.now();

  if (cache && now < cache.expires) {
    return cache.data as AcledResult;
  }
  ...
  const sanitizedEvents: AcledEvent[] = events.map((e: Record<string, unknown>) => ({
