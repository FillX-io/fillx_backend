/**
 * OpenRouter API Summarization Service
 * Fallback when Groq is rate-limited. Uses OpenRouter auto-routed free model.
 * In-memory cache with 24h TTL, shared key format with Groq.
 */
...
export async function openrouterSummarize(params: SummarizeParams): Promise<SummarizeResult | SummarizeError> {
  const apiKey = process.env.OPENROUTER_API_KEY;
  if (!apiKey) {
    return { summary: null, fallback: true, skipped: true, reason: 'OPENROUTER_API_KEY not configured', error: 'OPENROUTER_API_KEY not configured' };
  }
...
- Output ONLY the translated text.`;
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return { error: 'OpenRouter API error', fallback: true };
    }
