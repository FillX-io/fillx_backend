...
const COUNTRY_INDEX: Record<string, CountryIndex> = {
...
export async function fetchStockIndex(params: {
  code: string;
}): Promise<StockIndexResult> {
  ...
  if (!index) {
    return { available: false, code, error: "No stock index for country" };
  }

  const cacheKey = `stock-v1:${code}`;
  const cached = cacheMap.get(cacheKey);
  if (cached && Date.now() < cached.expires) {
    return cached.data as StockIndexResult;
  }
  ...
  if (!result) {
    return { available: false, code, error: "No data" };
  }
  ...
  if (allCloses.length < 2) {
    return { available: false, code, error: "Insufficient data" };
  }
  ...
  const payload: StockIndexResult = {
    available: true,
    code,
    symbol: index.symbol,
  ...
  cacheMap.set(cacheKey, { data: payload, expires: Date.now() + CACHE_TTL });
  return payload;
}
